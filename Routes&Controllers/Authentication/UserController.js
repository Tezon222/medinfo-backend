...
export const getUsers = async(req,res)=>{
  ...
  const users = await User.find().where("role").equals(`${role}`)
    ...
    role === "doctor" &&
     cloudinary.uploader.upload(req.file.path,{folder:"Event Featured images"}, async(err, result)=>{
      if(err){
        ...
        await User.create({firstName, lastName, email, password: securePassword, country, role:"Doctor", specialty, medicalCert: result.secure_url, gender, picture: avatar})
...
export const loginUser = async(req,res) =>{
    ...
    try {
        ...
        if(!user){
         ...
         return res.status(200).json({message: `Login Successful, welcome ${user.firstName}`})
        }else{
          return res.status(400).json({message: "Invalid username or password"})
        }
    } catch (error) {
