...
export const matchDoctor = async (req, res) =>{
    ...
    try{
        ...
        const doctors = await User.find({role: "Doctor"}).select("-logins -chatList -appointments -haveAppointment -password -__v") 
        ...
        return res.status(201).json({
            status: "Success",
            message: "Doctors Selected for Matching",
...
export const bookAppointment = async (req, res) =>{ 
    ...
    try{
        ...
        return res.status(400).json({
            status: "Error",
            message: "Unexpected error occurred while booking appointment",
            error: `${err}`
        })
    }
}
...
export const deleteAppointment = async (req, res) => {
    ...
    try{
        ...
        return res.status(200).json({
            ...
            zoomStatus: zoomMeeting
        })
    }catch(err){
        return res.status(400).json({ 
            status: "Error",
            message: "Unexpected error occurred while deleting appointment",
            error: `${err}`
        })
    }
}
